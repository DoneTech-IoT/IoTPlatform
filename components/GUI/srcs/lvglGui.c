...
void GUI_TaskInit(void)
{
    ...
    ////ESP_LOGE("LVGL", "Free Heap Size befor GUI: %u K bytes SRAM %u K byte PSRAM ", SRAM_Size, PSRAM_Size);
    ...
    ESP_LOGW("LVGL", " after gui creation : %u K bytes SRAM %u K byte PSRAM ", SRAM_Size, PSRAM_Size);
    PSRAM_Size = (esp_get_free_heap_size() / 1000);
    SRAM_Size = (xPortGetFreeHeapSize() / 1000);
    ////ESP_LOGE("LVGL", "Free Heap Size after GUI: %u K bytes SRAM %u K byte PSRAM ", SRAM_Size, PSRAM_Size);
...
void GUI_mainTask(void *pvParameter)
{
    ...
    PSRAM_Size = (esp_get_free_heap_size() / 1000);
    SRAM_Size = (xPortGetFreeHeapSize() / 1000);
    ////ESP_LOGE("LVGL", "Free Heap Size after LVGL_BigBuf: %u K bytes SRAM %u K byte PSRAM ", SRAM_Size, PSRAM_Size);
